...
```
```{r}
df_ml <- read.csv2("~/GitHub/Salmon-ai/Data/Dataset_MMA.csv")

```


```{r}
df_ml <- subset(df_ml, select = -c(Identifier, Source, LIS, HB, MCV
                                   ,MCH,MCHC, RDW, WBC, PLT, RETI, NEUTRO, RBC, LYMFO, MONO, BASO, EO, IJzergebrek, CRP, Anemie, FERRITINE, 
...
```{r}
colnames(df_ml)[0:2] <- c("Age", "Gender") 
print(df_ml)
```


```{r}
na_counts_base <- colSums(is.na(df_ml))
print(na_counts_base)
```

```{r}
write.csv(df_ml, "~/GitHub/Salmon-ai/Data/Cleaned_dataset_MMA_ML.csv")
...
```{r}
df_ml$deficiency <- ifelse(
  df_ml$VITB12 < 140 | (df_ml$VITB12 < 400 & df_ml$MMZ >= 300), 
  1, 
  0
)
...
df_ml <- df_ml %>%
  mutate(
    B12_deficient = ifelse(
      VITB12 < 140 | (VITB12 >= 140 & VITB12 <= 300 & MMZ >= 300 & eGFR >= 90), 
      1, 
      0
    )
  )
